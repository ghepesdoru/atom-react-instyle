...
export class AtomReactInstyle {
  ...
  activate(state) {
    ...
    console.log('In activation', state, this.settings);
  }
  ...
  convert(file, outputFormat, ignoreUnsupported) {
    ...
    this.convertor.convert(atom.workspace.getActiveTextEditor().buffer.cachedText, inputFormat, outputFormat).then((ret) => {
